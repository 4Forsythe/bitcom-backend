...
PATCH-запросы используются исключительно для **обновления** данных на сервере. Для работы с ними **обязательно** требуется **Basic Auth** токен.
...
- `/product-category` — обновление категорий на сервере

#### Body (только массив JSON)

|   Атрибут    | Значение                                                  |
| :----------: | --------------------------------------------------------- |
|    **id**    | **необязательная строка** идентификатора категории        |
|   **name**   | **необязательная строка** названия категории              |
| **parentId** | **необязательная строка** ссылки на id категории-родителя |
...
DELETE-запросы используются исключительно для **удаления** данных на сервере. Для работы с ними **обязательно** требуется **Basic Auth** токен.
...
- `/product-category` — удаление категорий на сервере

#### Param (ID категории)

- `/product-category/:id`, где id — обязательная строка идентификатора категории
